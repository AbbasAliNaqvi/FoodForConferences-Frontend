...
const CustomTabBarIcon = ({ focused, iconName, label }: any) => {
  ...
  useEffect(() => {
    Animated.parallel([
      Animated.timing(widthAnimation, { toValue: focused ? 110 : 50, duration: 250, useNativeDriver: false }),
      Animated.timing(opacityAnimation, { toValue: focused ? 1 : 0, duration: 150, useNativeDriver: false }),
      Animated.spring(scaleAnimation, { toValue: focused ? 1.05 : 1, friction: 6, tension: 100, useNativeDriver: true }),
    ]).start();
  }, [focused]);

  return (
    <Animated.View style={[ styles.tabIconContainer, { width: widthAnimation, backgroundColor: focused ? COLORS.light : 'transparent' } ]}>
  ...
  return (
    ...
    >
      ...
      <Tab.Screen
        ...
        options={{
          tabBarIcon: ({ focused }) => (
            <CustomTabBarIcon focused={focused} iconName="receipt" label="Orders" />
          ),
        }}
      />
      <Tab.Screen
      ...
      <Tab.Screen
        ...
        options={{
          tabBarIcon: ({ focused }) => (
            <CustomTabBarIcon focused={focused} iconName="person" label="Profile" />
          ),
        }}
      />
    </Tab.Navigator>
